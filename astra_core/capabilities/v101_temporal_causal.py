"""
V101 Temporal Causal Discovery -- compatibility module.

The V5.0 discovery capabilities were originally published as flat modules at
``astra_core.capabilities.v101_temporal_causal``. The subpackage reorganisation moved the
implementation to ``astra_core.capabilities.causal.temporal_causal`` but left nothing behind
at the old path, so every script written against the published API -- including
this repository's own ``tests/test_discovery/test_v5_capabilities.py`` -- broke
with ``ModuleNotFoundError``.

On a developer machine that also held older ASTRA checkouts the import could
still succeed by picking the module up from a *different tree*, which made the
breakage invisible locally while a clean clone from GitHub failed. This module
re-exports the canonical implementation so that a standalone install behaves
the same way.

There is no separate implementation here: these are the same objects.

    >>> from astra_core.capabilities.v101_temporal_causal import TemporalFCIDiscovery
    >>> from astra_core.capabilities.causal.temporal_causal import TemporalFCIDiscovery as canonical
    >>> TemporalFCIDiscovery is canonical
    True

New code should import from ``astra_core.capabilities.causal.temporal_causal``.
"""

from __future__ import annotations

from .causal.temporal_causal import (  # noqa: F401
    TemporalFCIDiscovery,
    TimeLaggedPAGEdge,
    GrangerFCIHybrid,
    create_temporal_fci_discovery,
    create_granger_fci_hybrid,
)

__all__ = [
    "TemporalFCIDiscovery",
    "TimeLaggedPAGEdge",
    "GrangerFCIHybrid",
    "create_temporal_fci_discovery",
    "create_granger_fci_hybrid",
]
